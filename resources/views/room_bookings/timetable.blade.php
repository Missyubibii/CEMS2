            ...
            <div class="inline-block min-w-full align-middle">
                <div class="bg-white overflow-x-auto shadow sm:rounded-lg">
                    <table class="table-auto w-full mt-4 divide-y divide-gray-200 dark:divide-gray-600">
                        <thead>
                            <tr>
                                <th scope="col" class="p-4 text-s tracking-wider text-left text-black uppercase ">Ngày đặt</th>
                                <th scope="col" class="p-4 text-s tracking-wider text-left text-black uppercase ">Tài khoản đặt</th>
                                <th scope="col" class="p-4 text-s tracking-wider text-left text-black uppercase ">Phòng - Tòa - Cơ sở</th>
                                <th scope="col" class="p-4 text-s tracking-wider text-left text-black uppercase ">Thời gian đặt</th>
                                <th scope="col" class="p-4 text-s tracking-wider text-left text-black uppercase ">Trạng thái đơn</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($roomBookings as $roomBooking)
                                <tr>
                                    <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">{{ $roomBooking->booking_date }}</td>
                                    <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">{{ $roomBooking->user->name }}</td>
                                    <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $roomBooking->room->room_name }} -
                                        {{ $roomBooking->room->building->building_name }} -
                                        {{ $roomBooking->room->campus->campus_name }}
                                    </td>
                                    <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">{{ $roomBooking->startPeriod->name }} - {{ $roomBooking->endPeriod->name }}</td>
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        <button class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300" data-modal-toggle="viewModal-{{ $roomBooking->id}}">
                                            Chi tiết
                                        </button>
                                    </td>
                                </tr>
                                    ...
                                    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                                        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                            ...
                                            <p class="block mb-2 text-base font-medium text-gray-900 dark:text-white">
                                                Ngày tạo đơn: {{ $roomBooking->created_at->format('d/m/Y') }}
                                            </p>
                                            ...
                                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                                <div class="sm:col-span-2" rows="5">
                                                    <p class="block mb-2 text-base font-medium text-gray-900 dark:text-white">
                                                        Người đặt:
                                                        <a class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                                                ...
                                                <p class="block mb-2 text-base font-medium text-gray-900 dark:text-white">
                                                    Địa chỉ phòng học:
                                                    <a class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                                                        Phòng {{ $roomBooking->room->room_name }} - Tòa {{ $roomBooking->room->building->building_name }} -
                                                        {{ $roomBooking->room->campus->campus_name }}
                                                    </a>
                                                </p>
                                                <p class="block mb-2 text-base font-medium text-gray-900 dark:text-white">
                                                ...
                                                <div class="sm:col-span-2" rows="5">
                                                    <p class="block mb-2 text-sm font-medium text-gray-900 ">
                                                        Mục đích:
                                                        <a class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500">
                                                            {{ $roomBooking->purpose }}
                                                        </a>
                                                    </p>
                                                </div>
