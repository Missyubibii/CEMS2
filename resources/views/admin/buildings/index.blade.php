<x-app-layout>
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 bg-white shadow sm:rounded-lg">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Danh sách tòa học</h2>

            <!-- Nút thêm mới -->
            <div class="flex justify-end mb-4">
                <button id="add_buildings_button" data-modal-target="add_buildings" data-modal-toggle="add_buildings"
                ...
                <table class="w-full text-sm text-left text-gray-500">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">Tên tòa học</th>
                    ...
                    <tbody>
                        @foreach($buildings as $building)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td class="px-6 py-4 font-semibold">{{ $building->id }}</td>
                                <td class="px-6 py-4">{{ $building->building_name }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button data-modal-target="deleteModal-{{ $building->id }}"
                                            data-modal-toggle="deleteModal-{{ $building->id }}"
                                        ...
                                        <!-- Modal xóa -->
                                        <div id="deleteModal-{{ $building->id }}" tabindex="-1"
                                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                            <div class="relative p-4 w-full max-w-md max-h-full">
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    <div class="p-4 md:p-5 text-center">
                                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                                            Bạn có chắc chắn muốn xóa tòa học này?
                                                        </h3>
                                                        <form action="{{ route('admin.buildings.destroy', $building) }}" method="POST" class="inline-flex">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                                            ...
                                                            <button type="button"
                                                                ...
                                                                data-modal-hide="deleteModal-{{ $building->id }}">
                                                                Hủy
                                                            </button>
                                                        </form>
                                                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            @if ($buildings->total() > 0)
                                ...
                                của {{ $buildings->total() }} tòa học
                            @else
                                Không có tòa học nào
                            @endif
                        </div>
                        <div class="withoutComponent">
                            {{ $buildings->links() }}
                        </div>
                    </div>
                </div>
            ...
            <!-- Form thêm mới -->
            <form id="add_buildings" tabindex="-1" aria-hidden="true"
                class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                            ...
                            <button type="button"
                                ...
                                data-modal-toggle="add_buildings">
                        ...
                        <div class="p-4 md:p-5">
                            <form method="POST" action="{{ route('admin.buildings.store') }}">
                                ...
                                <div class="grid gap-4 mb-4">
                                    <div>
                                        <label for="building_name" class="block mb-2 text-sm font-medium text-gray-900">
                                            Tên tòa học
                                        </label>
